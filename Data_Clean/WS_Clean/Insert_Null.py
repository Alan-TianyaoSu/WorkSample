...
# with open('Check4.txt', 'r') as file:
...
def increment_year(year_str):
    year_int = int(year_str)  # Convert the year to an integer
    incremented_year = year_int + 1  # Increment the year by one
    return str(incremented_year)  # Convert the incremented year back to a string
...
while last_db <= '2023':
    ...
    try:
        ...
        while True:
            ...
            if document_df.empty or Flag > END:
                print(last_db, ' 完成')
                last_db = increment_year(last_db)
                last_processed_position = 0
                break
            ...
            if not document_df.empty:
                ...
                operations = [InsertOne(doc) for doc in document_df.to_dict('records')]
                Target_collection.bulk_write(operations)
            end_4 = time.time()
            print(f"这批数据插入数据库时间：{end_4 - end_3} 秒")

            # 进度点保存
            with open('Check4.txt', 'w') as file:
    ...
    # 服务器异常
        ...
        print(e,'\n')
        last_processed_position = 0
        with open('Error_Check4.txt', 'a') as file:
            file.write(last_db)
